...
const Home = () => {
  ...
  const [clienteInfo, setClienteInfo] = useState<{ nombre: string; modelo: string; año: string; version: string } | null>(null);
  const [selectedServiceCategory, setSelectedServiceCategory] = useState<'ppf' | 'tratamiento-ceramico' | 'f-stack-black' | null>(null);
  ...
  const handleContinue = (info: { nombre: string; modelo: string; año: string; version: string }) => {
    setClienteInfo(info);
    setStep(2);
  };

  const handleSelectService = (service: 'ppf' | 'tratamiento-ceramico' | 'f-stack-black') => {
  ...
  const handleAddService = (service: Service, change: number) => {
    const currentServices = selectedServices[selectedServiceCategory as 'ppf' | 'tratamiento-ceramico'];
    const existingService = currentServices.find((s) => s.nombre === service.nombre);
    ...
    if (existingService) {
      ...
      setSelectedServices((prev) => ({
        ...prev,
        [selectedServiceCategory as 'ppf' | 'tratamiento-ceramico']: updatedServices,
      }));
    } else {
      setSelectedServices((prev) => ({
        ...prev,
  ...
  const handleRemoveService = (service: Service) => {
    setSelectedServices((prev) => ({
      ...prev,
      [selectedServiceCategory as 'ppf' | 'tratamiento-ceramico']: prev[selectedServiceCategory as 'ppf' | 'tratamiento-ceramico'].filter(
  ...
  const total = selectedServices.ppf.reduce((acc, service) => acc + service.precio * service.cantidad, 0) +
    selectedServices['tratamiento-ceramico'].reduce((acc, service) => acc + service.precio * service.cantidad, 0) +
    selectedServices['f-stack-black'].reduce((acc, service) => acc + service.precio, 0);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center bg-black text-white">
      ...
      <div className={getLogoMargin()}>
        <img src="images/logo.png" alt="Logo F-Stack" className="h-auto max-h-44 w-auto" />
      </div>

      {/* Contenido principal */}
      <div>
        {step === 1 && <StepOne onContinue={handleContinue} />}
        {step === 2 && <ServiceSelection onSelectService={handleSelectService} />}
